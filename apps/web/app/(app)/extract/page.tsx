...
export default function ExtractPage() {
  ...
  const addFiles = useCallback((list: FileList | null) => {
    ...
    if (ok.length < list.length) {
      setError("Một số file bị bỏ qua (chỉ nhận MP4/MOV/MKV/WebM, tối đa 2GB).");
    }
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      <div className="text-center">
        ...
        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
          Tách phụ đề từ video (chữ trên hình hoặc giọng nói) ra file SRT — không dịch.
          Xong vào video để tải file về.
        </p>
      </div>
      ...
      >
        ...
        <p className="mt-3 text-sm font-medium">Kéo thả video/âm thanh hoặc bấm để chọn</p>
        <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">
          MP4, MOV, MKV, WebM — tối đa 2GB/file
        </p>
        <input
          type="file"
          multiple
      ...
      >
        {running ? (
          ...
          <>
            <Play className="h-4 w-4" /> Bắt đầu trích xuất
          </>
        )}
      </button>
