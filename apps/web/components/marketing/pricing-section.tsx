...
export function PricingSection() {
  return (
    <section id="bang-gia" className="mx-auto max-w-6xl scroll-mt-20 px-4 py-16">
      ...
      {/* đơn giá dịch vụ */}
      <div className="mx-auto mt-10 max-w-2xl rounded-2xl border border-white/5 bg-white/[0.03] p-5">
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {PACKS.map((p) => (
          <div
            ...
            className={
              p.popular
                ? "relative rounded-2xl border border-indigo-500/60 bg-indigo-500/[0.08] p-5"
                : "relative rounded-2xl border border-white/5 bg-white/[0.03] p-5"
            }
          >
            {p.popular && (
            ...
            <Link
              ...
              className={
                p.popular
                  ? "mt-5 block rounded-xl bg-indigo-600 px-4 py-2.5 text-center text-sm font-semibold text-white hover:bg-indigo-500"
                  : "mt-5 block rounded-xl border border-white/10 px-4 py-2.5 text-center text-sm font-semibold text-neutral-200 hover:bg-white/5"
              }
            >
              Nạp ngay
            </Link>
